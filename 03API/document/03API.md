...
​		③ int number = r.nextInt(n);(生成 [0,n) 的随机数,包左不包右)
...
​	String类实际上是一个用final修饰的字符数组, 更底层实际上就是final修饰的字节数组
...
​	public static boolean isNull(Object obj)：判断变量是否为null ,为null返回true ,否则, 返回false
...
​		当参数是引用数据类型时 按引用传递 实参和形参执行堆内存上同一个地址 修改形参会对实参产生影响，但是包装类不是！
...
​	使用 Javadoc 生成的 API 接口文档是 html 格式
