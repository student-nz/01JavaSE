...
​		集合是Java中存储对象数据的一种容器，因为集合只支持引用类型, 不支持基本数据类型！
...
​		在Java中, 集合分单列集合和双列集合两类

​		单列集合的顶级父类容器为Collection, 且每个元素（数据）只包含一个值

​		双列集合的顶级父类容器为Map,  且每个元素包含两个值（键值对形式）

​				  ![image-20221012084756602](assets\集合体系结构.png)

​	

## 4.1 Collection集合体系

​	Collection是单列集合的祖宗接口，它的功能是全部单列集合都可以继承使用的			![image-20221012085224808](assets\Collection集合体系.png)

### 	4.1.1 Collection集合特点
...
​			ArrayList（动态数组）：有序，可重复，有索引（ 在 API 章节已经介绍了ArrayList基本使用）						 
...
​				增强for修改变量修改第三方变量的值不会影响到集合中的元素			![image-20221012092210314](assets\增强for修改变量.png)
...
​	实例如下：![image-20221012092454615](assets\Collection结合Lambda遍历的API.png)

### 	4.1.4 Collection集合存储自定义类型的对象

​			不做解释, 只供思考!

​			提示：自定义类型就是引用类型				 

### 	4.1.5 Collection系列集合之List系列集合
...
​							![image-20221012134516868](assets\泛型运行期才报问题.png)

​							![image-20221012134756374](assets\泛型编译报错异常.png)							
...
​							![image-20221012134821522](assets\泛型报错提到编译期.png)
...
​				如果没有重写hashCode方法，那么是跟据对象的地址值计算出的哈希值，

​				同一对象多次调用hashCode()方法返回的哈希值是相同的，不同对象的哈希值是不一样的！

​				

​				我们可以对Object类中的hashCode方法进行重写，在重写之后，我们一般是通过对象的属性值来计算哈希值的，
...
![image-20221013083523270](assets\HashSet1.7版本原理解析.png)

#### 	7. JDK1.8版本开始HashSet原理解析

​		底层结构：哈希表（数组、链表、红黑树的结合体）

​		当挂在元素下面的数据过多时, 查询性能降低, 从JDK8开始后, 当链表长度超过8的时候, 自动转换为红黑树

![image-20221013083916397](assets\JDK1.8版本开始HashSet原理解析.png)
...
![image-20221013084924105](assets\HashSet元素去重复的底层原理.png)
...
​				![image-20221013090736031](assets\LinkedHashSet集合.png)
...
​			![image-20221013123902349](assets\Map集合体系.png)
...
![image-20221013130337245](assets\HashMap的添加规则.png)
...
​		![image-20221013131520917](assets\LinkedHashMap的添加规则.png)
...
​			TreeMap跟TreeSet底层原理一样（都是基于红黑树实现排序，增删改查性能较好）
...
![image-20221012102511852](assets\二叉查找树.png)
...
​									 ![image-20221012102803107](assets\二叉树查找存在的问题：.png)
...
​							![image-20221012103020978](assets\平衡二叉树.png)
...
![image-20221012103759078](assets\红黑树.png)
